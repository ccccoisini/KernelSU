		     ...
		     unsigned long arg4, unsigned long arg5)
{
	...
	if (current_uid().val == 0) {
		...
		if (arg2 == CMD_SUSFS_ADD_SUS_MAPS) {
			if (access_ok((void __user*)arg3, sizeof(struct st_susfs_sus_maps))) {
		...
		if (arg2 == CMD_SUSFS_ADD_SUS_PROC_FD_LINK) {
			if (access_ok((void __user*)arg3, sizeof(struct st_susfs_sus_proc_fd_link))) {
		...
		if (arg2 == CMD_SUSFS_ADD_SUS_MEMFD) {
			if (access_ok((void __user*)arg3, sizeof(struct st_susfs_sus_memfd))) {
...
int ksu_handle_setuid(struct cred *new, const struct cred *old)
{
	...
	try_umount("/system_k", false, MNT_DETACH);
